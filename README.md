...
_.set('巨大娘.络络.变化原因', '喝下药水') // 可选：变化原因
_.set('巨大娘.络络.变化时间', '第三天')  // 可选：变化时间点
```
...
_.set('巨大娘.络络.当前身高', 170);
_.set('巨大娘.络络.原身高', 1.65);

// 单独设置胸部尺寸为 28 米（约 200 倍，比整体更大）
_.set('巨大娘.络络.自定义部位.乳房高度', 28);

// 单独设置脚掌为 40 米（约 160 倍）
_.set('巨大娘.络络.自定义部位.足长', 40);
```
...
可用场景：荒野、乡村、郊区、小城市、中等城市、大城市、超大城市中心、东京市中心、香港、马尼拉、住宅内、公寓楼内、办公楼内、体育馆内、巨大娘体内
...
```javascript
// 记录累计损害
_.set('巨大娘.络络._实际损害.总伤亡人数', 15000);
_.set('巨大娘.络络._实际损害.总建筑损毁', 50);

// 记录最近一次行动
_.set('巨大娘.络络._实际损害.最近行动', {
  描述: '在市中心踩下一脚',
  伤亡人数: 500,
  建筑损毁: 5
});
