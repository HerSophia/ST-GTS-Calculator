...
```javascript
// 记录累计损害
_.set('巨大娘.络络._实际损害.总伤亡人数', 15000);  // 累计伤亡
_.set('巨大娘.络络._实际损害.总建筑损毁', 50);     // 累计损毁建筑
_.set('巨大娘.络络._实际损害.总城市毁灭', 1);      // 累计毁灭城市
...
当 AI 需要更新角色身高时，会按照以下格式输出：

```javascript
_.set('巨大娘.络络.当前身高', 500);  // 新身高（米）
_.set('巨大娘.络络.变化原因', '喝下成长药水');
_.set('巨大娘.络络.变化时间', '第三天傍晚');
```
